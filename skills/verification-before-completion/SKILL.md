...
5. Invoke `superpowers:code-review` only when the user explicitly asks for an
   implementation review. Risk, complexity, file or line count, and agent
   confidence are not triggers. Do not invoke it again when the conversation
   already contains a Review Handoff for the current scope.
